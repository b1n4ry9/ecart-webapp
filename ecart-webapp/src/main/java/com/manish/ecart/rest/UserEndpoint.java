...
import com.manish.ecart.rest.dto.UserDTO;
...
public class UserEndpoint {
	...
	@GET
	@Produces("application/json")
	public List<UserDTO> listAll(@QueryParam("start") Integer startPosition,
	...
	@GET
	@Path("/login")
	public Response isValidUser(@QueryParam("userName") String userName,@QueryParam("userPassword") String password) {
		System.out.println(userName);
		TypedQuery<User> validUserQuery = em.
				createQuery(
						"SELECT u FROM User u WHERE u.userName = :userName AND u.userPassword = :userPassword",
						User.class);
		validUserQuery.setParameter("userName", userName);
		validUserQuery.setParameter("userPassword", password);
		Integer userCount = validUserQuery.getResultList().size();
